...
export class RepositoryController {
  ...
  @Post("analyze") analyze(
    ...
    @Body() body: AnalyzeRepositoryDto,
  ) {
    return this.repository.analyze(
      user,
      projectId,
      body.repositoryUrl,
      body.branch,
  ...
  @Get("analysis") latest(
    ...
    return this.repository.latest(user.id, projectId);
  }
  @Post("generate") generate(
    @CurrentUser() user: AuthenticatedUser,
    @Param("projectId") projectId: string,
  ) {
    return this.repository.generate(user, projectId);
  }
}

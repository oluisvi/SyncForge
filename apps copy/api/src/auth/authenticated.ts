...
export interface AuthenticatedUser { readonly id: string; readonly email: string; }
...
export class SessionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const token = parseCookie(headerValue(request.headers.cookie), SESSION_COOKIE) ?? bearer(headerValue(request.headers.authorization));
    if (!token) throw new ApiUnauthorized();
    const session = await this.sessions.authenticate(token);
    if (!session) throw new ApiUnauthorized("SESSION_EXPIRED", "Session expired or invalid");

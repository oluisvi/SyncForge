import { booleanValue, enumValue, integerValue, loadEnvironment, optional, stringValue, urlValue, withDefault } from "@syncforge/config";
...
const httpOrigin: EnvironmentRule<string> = {
  parse(value, key) {
    ...
    if (parsed.username || parsed.password || parsed.pathname !== "/" || parsed.search || parsed.hash) {
      throw new Error(`${key} must be an origin without credentials, path, query, or fragment`);
    }
